...
- `components/AnalyticsDashboard.tsx` — reads `ExamAttempt[]`, flattens into per-question events (correct / wrong / flagged), aggregates across week / cognitive level / cluster / LO / source document. Renders the hierarchical "Where to focus next" drilldown plus a Review Questions tab for browsing past wrong/flagged stems. Expanded review rows embed `<QuestionTutorPanel>` so Deep Dive + AI Tutor chat work on historical questions too.
- `components/QuestionTutorPanel.tsx` — **canonical shared component for Deep Dive + AI Tutor chat + "Ask about this" selection popup.** Owns all its own state (`deepDiveContent`, `chatMessages`, `selectionPopup`). Takes `contentRef` from parent so selection-detection scope is parent-controlled. Also exports `AssistantText` (markdown-lite renderer for **bold** headers + `- ` bullets). Use this wherever Deep Dive / chat is needed — do NOT reimplement the Deep Dive+chat UI inline.
...
- **"Ask about this" selection popup** is scoped to the parent-provided `contentRef` passed to `<QuestionTutorPanel>`. That ref must wrap the question content (vignette + lead-in + options + explanation) AND the tutor panel itself. Selections inside the panel's own chat thread are automatically excluded (via internal `chatPanelRef`) so users can copy from replies freely. The popup uses `position: fixed` with viewport coords from `range.getBoundingClientRect()` — works across scroll containers without overflow-clipping issues.
- **Where `<QuestionTutorPanel>` is used:** post-submit `QuestionCard` (in the explanation block) AND the expanded review rows in `AnalyticsDashboard`'s Review Questions tab. Both paths use the same `handleDeepDive` / `handleChatSend` handlers from `App.tsx`. If you add a third place that renders past questions, embed `<QuestionTutorPanel>` with its own `contentRef`; do NOT inline a copy of the chat/Deep-Dive UI.
